...
# 视频文件路径
VIDEO_FILE="/Users/shhaofu/Downloads/绝美天堂_隐藏的自然秘境_4K_HDR_杜比_-_放松风光影片.32629983713.mp4"
# 输出图片格式
OUTPUT_FORMAT="captured/output_%03d.jpg"
...
# 检查命令执行是否成功
if [ $? -eq 0 ]; then
    # 统计生成的图片数量（这个检查有bug TODO 因为OUTPUT_FORMAT并不是通用的正则表达式，和普通的正则表达式有一点点差异或者我们没有正确使用，暂时忽略这个提示：生成0个图片的问题）
