        ...
        self.effects = [
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    # 水平平移滤镜：从左到右缓慢移动图片
                    # z=1: 缩放比例保持1倍（不缩放）
                    # x=(iw-w)*on/den: 水平位置随时间线性变化，从左到右
                    # y=(ih-h)/2: 垂直位置保持居中
                    # d=1: 每帧处理一次
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    # 将图片放大到至少覆盖目标画布
                    base_upscale,
                    # Ken Burns效果：缓慢缩放并保持中心位置
                    # z=1.0+0.15*on/den: 缩放比例从1.0逐渐增加到1.15
                    # x='iw/2-(ow*zoom/2)': 水平位置保持中心
                    # y='ih/2-(oh*zoom/2)': 垂直位置保持中心
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    # 设置输出帧率
                    f"fps={self.output_fps}",
                    # 使用colorchannelmixer滤镜混合颜色通道，模拟复古棕褐色调
                    # 参数分别对应RGB通道的混合比例，创造温暖的复古效果
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    # 设置输出帧率
                    f"fps={self.output_fps}",
                    # 使用drawtext滤镜在左上角添加文字水印
                    # text='My Slideshow': 水印文字内容
                    # x=10:y=10: 文字位置（距离左上角的偏移）
                    # fontsize=36: 字体大小
                    # fontcolor=white: 字体颜色
                    # box=1:boxcolor=black@0.35:boxborderw=10: 文字背景框设置
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    f"fps={self.output_fps}",
                    # 使用drawtext滤镜添加动态帧计数
                    # text='Frame %{n}': 使用%{n}变量显示当前帧号
                    # x=w-tw-20:y=20: 文字位置（右上角，留出边距）
                    "drawtext=text='Frame %{n} timestamp %{pts}':x=w-tw-20:y=20:fontsize=28:fontcolor=red:box=1:boxcolor=black@0.35:boxborderw=10"
                ])
            },
            {
                "name": "slow_motion",
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    # 设置输出帧率
                    f"fps={self.output_fps}",
                    # 第一次缩放：将图像缩小到1/20大小，使用neighbor插值方法保留块状效果
                    "scale=iw/20:ih/20:flags=neighbor",
                    # 第二次缩放：将缩小的图像放大回原始大小，保持块状效果，形成马赛克
                    "scale=iw*20:ih*20:flags=neighbor"
                ])
            },
            {
            ...
            },
            {
                ...
                "filter": self._vf_with_duration([
                    ...
                    # 淡入效果：从第0帧开始，持续约0.5秒
                    f"fade=in:0:{max(1, int(self.output_fps*1.5))}",
                    # 淡出效果：在结束前0.5秒开始，持续约0.5秒
                    f"fade=out:{max(0, self.total_frames - int(self.output_fps*1.5))}:{max(1, int(self.output_fps*1.5))}",
                    # 在底部居中位置添加文字
